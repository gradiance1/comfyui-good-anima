...
description: |
  ...
  不用于已经给出主体、场景、动作/关系、镜头或构图、风格/光影等完整画面方案的 prompt。
---
...
拿到用户需求后，先做意图识别：用户到底想让画面表达什么，哪些内容已经被明确锁定，哪些缺口会影响画面成立。只有用户已经给出完整画面方案时，才跳过展开；否则把模糊需求拆成三层，再落回构图。意图层的目标是把模糊需求变成清楚需求，不是替用户强加固定设定。不要字面翻译用户语句：用户说“花海”不等于只写 `flower field`；用户没说“手势”也不等于人物一定要站着。意图层负责展开，规则层负责收缩；不要在这里写 Anima 崩点案例。
...
- 用户已有完整 prompt，且主体、场景、动作/关系、镜头或构图、风格/光影已经足够支撑画面成立。
