...
2. 随机图 / roll / 抽卡（包含随机 tag / 随机画师 / 随机构图等多维度随机，用户说"随机/roll/抽卡/来一张随机的"）→ `anima-random-gen` 产出参数 → 校验 → 通过 `comfyui-manager` 执行
3. 仅随机画师（其他由用户指定，用户说"随机画师/抽个画师"）→ `danbooru-tags --random 5 --for-prompt --json --compact` → 选 1 个画师 → 组装参数 → 通过 `comfyui-manager` 执行
...
- 批量 `submit` 后只汇总每张编号、简短主题、`prompt_id` 和 manifest；后处理完成后才汇总 `local_path`。不要把完整 prompt 和检索 JSON 全部复述给用户。
